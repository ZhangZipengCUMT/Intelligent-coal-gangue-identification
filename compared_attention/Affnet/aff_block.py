...
from Affnet.mobilenetv2 import InvertedResidual
from Affnet.base_module import BaseModule
from Affnet.profiler import module_profile
...
class AFNO2D_channelfirst(nn.Module):
    ...
    def __init__(self, cfg, hidden_size, num_blocks=8, sparsity_threshold=0.01, hard_thresholding_fraction=1,
                 hidden_size_factor=1):
        super().__init__()
        assert hidden_size % num_blocks == 0, f"hidden_size {hidden_size} should be divisble by num_blocks {num_blocks}"

        self.hidden_size = hidden_size
        self.sparsity_threshold = cfg.AFNO2D_sparsity_threshold
        #getattr(opts, "model.activation.sparsity_threshold", 0.01)
        ...
        self.act = self.build_act_layer(cfg=cfg)
        self.act2 = self.build_act_layer(cfg=cfg)

    @staticmethod
    def build_act_layer(cfg) -> nn.Module:
        act_type = cfg.AFNO2D_name
        #getattr(opts, "model.activation.name", "relu")
        neg_slope = cfg.AFNO2D_neg_slope
        #getattr(opts, "model.activation.neg_slope", 0.1)
        inplace = cfg.AFNO2D_activation_inplace
        #getattr(opts, "model.activation.inplace", False)
...
class Block(nn.Module):
    def __init__(self, cfg, dim, hidden_size, num_blocks, double_skip, mlp_ratio=4., drop_path=0., attn_norm_layer='sync_batch_norm', enable_coreml_compatible_fn=False):
        # input shape [B C H W]
        super().__init__()
        self.norm1 = get_normalization_layer(
            cfg=cfg, norm_type=attn_norm_layer, num_features=dim
        )
        self.filter = AFNO2D_channelfirst(cfg=cfg, hidden_size=hidden_size, num_blocks=num_blocks, sparsity_threshold=0.01,
                                          hard_thresholding_fraction=1, hidden_size_factor=1)
        # if not enable_coreml_compatible_fn else \
        # AFNO2D_channelfirst_coreml(opts=opts, hidden_size=hidden_size, num_blocks=num_blocks,
        # sparsity_threshold=0.01, hard_thresholding_fraction=1, hidden_size_factor=1)
...
class AFFBlock(BaseModule):
    ...
    def __init__(
        ...
        self.enable_coreml_compatible_fn = cfg.Enable_coreml_compatible_module \
                                           or cfg.Use_jit_model
...
if __name__ == '__main__':
    ...
    from config import AffnetConfig
    cfg = AffnetConfig()
    device = torch.device("cuda:0")
    net = Block(cfg, dim=1, hidden_size=1, num_blocks=1, double_skip=True,attn_norm_layer="batch_norm").to(device)
    # net = AFFBlock(cfg, in_channels=1, transformer_dim=1, ffn_dim=256)#.to(device)
    inx = torch.randn(2, 1, 128, 600).to(device)
    r = net(inx)

    # for i in tqdm(range(1, 300)):
    #     for j in range(1, 300):
    #         for z in range(1, 300):
    #             inx = torch.randn(2, i, j, z).to(device)
    #             try:
    #                 r = net(inx)
    #                 print(i, j, z)
    #             except:
