...
from Model_SimAM import Proposed_model_v1, Proposed_model_v3
...
model = Proposed_model_v3(nums=2, RetNet_param=RetNet_param,
                           ...
                           cls_lnum=1, cls_linearfeas=504, class_num=2, N_nums=3)
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        print('train time',time2-time1,'train time',time4-time2)
        save_model(model, folder=save_folder, epoch=epoch)
        his.append([epoch, loss, acc, test_acc])
    pd.DataFrame(his, columns=["epoch", "loss", "acc", "test_acc"]).to_csv(os.path.join(save_folder, "Train_simam .csv"))
