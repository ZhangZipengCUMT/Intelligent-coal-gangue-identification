...
from Affnet.layers.normalization_layers import get_normalization_layer
...
class ConvLayer(BaseLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_norm:
            norm_type = cfg.Invres_norm_name
        ...
        self.norm_name = None
        if use_norm:
            norm_layer = get_normalization_layer(cfg=cfg, num_features=out_channels)
            block.add_module(name="norm", module=norm_layer)
            self.norm_name = norm_layer.__class__.__name__

        self.act_name = None
        act_type = (
            cfg.Invres_activation_name
            if act_name is None
            else act_name
        )

        if act_type is not None and use_act:
            neg_slope = cfg.Invres_neg_slope
            inplace = cfg.Invres_activation_inplace
